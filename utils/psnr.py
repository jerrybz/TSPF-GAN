...
def wpsnr(compare_tesnors: Sequence[torch.Tensor], weight_map: torch.Tensor = None, max_val: float = 1.) -> torch.Tensor:
    ...
    ---
        ValueError: If `compare_tensors` and `weight_map` have different shapes, or if they are not 4D tensors with
            the expected shapes.
    """
    if weight_map is None or torch.all(weight_map == 0): # if no weight map is provided, use ones | this will be the same is normal SSIM which uses torch.mean()
    ...
    if len(weight_map.shape) != 4 or len(y.shape) != 4:
        raise ValueError(f"Expected weight_map, y and y_pred to be 4D tensors. Got weight_map: {weight_map.shape} and y: {y.shape}, and y_pred: {y_pred.shape}.")
